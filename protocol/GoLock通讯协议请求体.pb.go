// Code generated by protoc-gen-go. DO NOT EDIT.
// source: convention/protobuf/GoBlog通讯协议请求体.proto

package protocol

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

type LockType int32

const (
	LockType_UPDATE LockType = 0
	LockType_SHARE  LockType = 1
)

var LockType_name = map[int32]string{
	0: "UPDATE",
	1: "SHARE",
}

var LockType_value = map[string]int32{
	"UPDATE": 0,
	"SHARE":  1,
}

func (x LockType) String() string {
	return proto.EnumName(LockType_name, int32(x))
}

func (LockType) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_635b12b7812ec00d, []int{0}
}

type GoBlogRequestBody struct {
	LockName             string   `protobuf:"bytes,1,opt,name=lock_name,json=lockName,proto3" json:"lock_name,omitempty"`
	LockType             LockType `protobuf:"varint,2,opt,name=lock_type,json=lockType,proto3,enum=protocol.LockType" json:"lock_type,omitempty"`
	LockTime             uint32   `protobuf:"varint,3,opt,name=lock_time,json=lockTime,proto3" json:"lock_time,omitempty"`
	WaitTime             uint32   `protobuf:"varint,4,opt,name=wait_time,json=waitTime,proto3" json:"wait_time,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *GoBlogRequestBody) Reset()         { *m = GoBlogRequestBody{} }
func (m *GoBlogRequestBody) String() string { return proto.CompactTextString(m) }
func (*GoBlogRequestBody) ProtoMessage()    {}
func (*GoBlogRequestBody) Descriptor() ([]byte, []int) {
	return fileDescriptor_635b12b7812ec00d, []int{0}
}

func (m *GoBlogRequestBody) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_GoBlogRequestBody.Unmarshal(m, b)
}
func (m *GoBlogRequestBody) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_GoBlogRequestBody.Marshal(b, m, deterministic)
}
func (m *GoBlogRequestBody) XXX_Merge(src proto.Message) {
	xxx_messageInfo_GoBlogRequestBody.Merge(m, src)
}
func (m *GoBlogRequestBody) XXX_Size() int {
	return xxx_messageInfo_GoBlogRequestBody.Size(m)
}
func (m *GoBlogRequestBody) XXX_DiscardUnknown() {
	xxx_messageInfo_GoBlogRequestBody.DiscardUnknown(m)
}

var xxx_messageInfo_GoBlogRequestBody proto.InternalMessageInfo

func (m *GoBlogRequestBody) GetLockName() string {
	if m != nil {
		return m.LockName
	}
	return ""
}

func (m *GoBlogRequestBody) GetLockType() LockType {
	if m != nil {
		return m.LockType
	}
	return LockType_UPDATE
}

func (m *GoBlogRequestBody) GetLockTime() uint32 {
	if m != nil {
		return m.LockTime
	}
	return 0
}

func (m *GoBlogRequestBody) GetWaitTime() uint32 {
	if m != nil {
		return m.WaitTime
	}
	return 0
}

func init() {
	proto.RegisterEnum("protocol.LockType", LockType_name, LockType_value)
	proto.RegisterType((*GoBlogRequestBody)(nil), "protocol.GoBlogRequestBody")
}

func init() {
	proto.RegisterFile("convention/protobuf/GoBlog通讯协议请求体.proto", fileDescriptor_635b12b7812ec00d)
}

var fileDescriptor_635b12b7812ec00d = []byte{
	// 224 bytes of a gzipped FileDescriptorProto
	,
}
